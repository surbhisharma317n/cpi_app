...
export const fetchFinalInputPriceItem = createAsyncThunk(
  "base/final_input_price_item",
  async (
    ...
    try {
      ...
      return await capiService.getFinalInputPriceItem(tab, dataType); // <-- pass here
    } catch (err: any) {
...
const baseSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
    .addCase(fetchFinalInputPriceItem.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchFinalInputPriceItem.fulfilled, (state, action: PayloadAction<any[]>) => {
        state.loading = false;
        state.baseData = action.payload;
      })
      .addCase(fetchFinalInputPriceItem.rejected, (state, action) => {
        state.loading = false;
