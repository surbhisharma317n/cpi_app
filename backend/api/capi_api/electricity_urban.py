...
from ..config import API_ELECTRICITY_URBAN_DATA_URL, URBAN_ELECTRICITY_TABLE_NAME
from api.capi_api.capi_utils import airfare_data_exists, login_and_get_token, fetch_capi_data, save_to_table
# Configuration
# TABLE_NAME = "urban_electricity"

urban_electricity_column_mapping = {
        "user_id": "user_id",
...
@api_view(['GET'])
def fetch_and_insert_urban_electricity_prices(request):
    print("Fetching and inserting urban electricity prices...")
    try:
        ...
        if airfare_data_exists(survey_date, URBAN_ELECTRICITY_TABLE_NAME):
            return JsonResponse({
                "status": "success",
        ...
        inserted_rows = save_to_table(data, urban_electricity_column_mapping, URBAN_ELECTRICITY_TABLE_NAME)
        return JsonResponse({
            "status": "success",
