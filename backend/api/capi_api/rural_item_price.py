...
from ..config import API_MARKET_RURAL_PRICES_DATA_URL, RURAL_ITEM_PRICE_TABLE_NAME

# Configuration
# RURAL_ITEM_PRICE_TABLE_NAME = "rural_item_price"

rural_item_prices_column_mapping = {
    ...
    "local_area_code": "item_local_area_code",
    ...
    # Extra DB fields
    "variety": "variety",         # API may not have this → keep as None
    "item_spec": "item_spec",     # API may not have this → keep as None
}



@api_view(['POST'])
def fetch_rural_item_price_data_view(request):
    try:
        ...
        if airfare_data_exists(survey_date, RURAL_ITEM_PRICE_TABLE_NAME):
            return JsonResponse({
                "status": "success",
        ...
        while True:
            ...
            page_data = fetch_capi_data_from_post_api(
                token,
                survey_date,
                API_MARKET_RURAL_PRICES_DATA_URL,
                page_number,
                page_size
            )
        ...
        inserted_rows = save_to_table(all_data, rural_item_prices_column_mapping, RURAL_ITEM_PRICE_TABLE_NAME)
        return JsonResponse({
            "status": "success",
