...
from ..config import API_HOUSERENT_RURAL_DATA_URL, RURAL_HOUSE_RENT_TABLE_NAME

# Configuration
# RURAL_HOUSE_RENT_TABLE_NAME = "rural_housing_rent"

rural_house_rent_column_mapping = {
...
@api_view(['POST'])
def fetch_rural_house_rent_data_view(request):
    try:
        ...
        # data = fetch_capi_data(token, survey_date, API_HOUSERENT_RURAL_DATA_URL)
        if airfare_data_exists(survey_date, RURAL_HOUSE_RENT_TABLE_NAME):
            return JsonResponse({
                "status": "success",
        ...
        inserted_rows = save_to_table(all_data, rural_house_rent_column_mapping, RURAL_HOUSE_RENT_TABLE_NAME)
        return JsonResponse({
            "status": "success",
